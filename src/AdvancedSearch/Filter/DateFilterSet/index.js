...
const DateFilterSet = props => {
  const renderDateConfig = props => {
    switch (props.paramMapping.comparison) {
      case 'lte':
        break
      case 'gte':
        break
      ...
      <Row>
        <Col>
          <CustomSelect
            ...
            size="sm"
            value={props.options.find(option => option.value === props.paramMapping.value)}
          />
        </Col>
        <Col className="date-field">{renderDateConfig(props)}</Col>
      </Row>
    </Col>
  )
}

class OldDateFilter extends React.Component {
  constructor(props) {
    super(props)
